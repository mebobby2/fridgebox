...
APP_KEY = "1wzncpcu43zs86f"
APP_SECRET = "hay5vby5kdkmby3"
...
class DropboxController < ApplicationController
    ...
    def list
        ...
        #assume all files are stored in the root dir
        @files = root_metadata['contents'].map { |f| {name: f['path'][1..-1]} }
    end

    def download
        client = get_dropbox_client
        content = client.get_file("/#{params[:file]}")
        send_data(content, filename: params[:file])
    end
